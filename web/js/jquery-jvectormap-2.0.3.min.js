...
(function (factory) {
    ...
    function handler(event) {
        ...
        if (0 !== deltaY || 0 !== deltaX) {
            ...
            if (!lowestDelta || lowestDelta > absDelta) {
                lowestDelta = absDelta;
                if (special.settings.adjustOldDeltas && "mousewheel" === orgEvent.type && absDelta % 120 === 0) {
                    lowestDelta /= 40;
                }
            }
            if (special.settings.adjustOldDeltas && "mousewheel" === orgEvent.type && absDelta % 120 === 0) {
...
jvm.VMLShapeElement.prototype.applyAttr = function (attr, value) {
    switch (attr) {
        ...
        case "stroke-opacity":
            ...
            break;
        case "stroke-width":
            this.node.stroked = !(0 === parseInt(value, 10));
...
jvm.VMLPathElement.pathSvgToVml = function (path) {
    ...
    return path.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function (segment, letter, coords) {
        coords = coords.replace(/(\d)-/g, "$1,-").replace(/^\s+/g, "").replace(/\s+$/g, "").replace(/\s+/g, ",").split(",");
        if (!coords[0]) {
            coords.shift();
        }
        var i = 0;
        for (var l = coords.length; l > i; i++) {
...
jvm.NumericScale.prototype = {
    ...
    getValue: function (value) {
        ...
        i = 0;
        for (value -= this.minValue; value - lengthes[i] >= 0;) {
            value -= lengthes[i];
            i++;
        }
    ...
    getTicks: function () {
        ...
        for (tick = extent[0]; tick <= extent[1];) {
...
jvm.DataSeries = function (params, elements, map) {
    ...
    if (jvm.$.isArray(params.scale)) {
        scaleConstructor = "fill" === params.attribute || "stroke" === params.attribute ? jvm.ColorScale : jvm.NumericScale;
        ...
        if ("string" == typeof key) {
            ...
            for (code in key)
                if (this.elements[code]) {
                    this.elements[code].element.setStyle(this.params.attribute, key[code]);
                }
        }
    },
...
jvm.Proj = {
    ...
    aea: function (lat, lng, c) {
        ...
        var ro0 = Math.sqrt(C - 2 * n * Math.sin(0)) / n;
        return {
    ...
    lcc: function (lat, lng, c) {
        ...
        var ro0 = F * Math.pow(1 / Math.tan(Math.PI / 4 + 0), n);
        return {
...
jvm.MapObject.prototype.getLabelOffsets = function (key) {
    ...
    if (this.config.label) {
        if ("function" == typeof this.config.label.offsets) {
            offsets = this.config.label.offsets(key);
        } else if ("object" == typeof this.config.label.offsets) {
            offsets = this.config.label.offsets[key];
        }
    }
    return offsets || [0, 0];
...
jvm.Map = function (params) {
    ...
    if (this.params.focusOn) {
        if ("string" == typeof this.params.focusOn) {
            this.params.focusOn = {
...
jvm.Map.prototype = {
    ...
    bindContainerEvents: function () {
        ...
        if (this.params.zoomOnScroll) {
            this.container.mousewheel(function (event) {
                ...
                var zoomStep = Math.pow(1 + map.params.zoomOnScrollSpeed / 1e3, event.deltaFactor * event.deltaY);
                map.tip.hide();
    ...
    bindContainerPointerEvents: function () {
        ...
        var handleGestureEvent = function (e) {
            var transXOld;
            var transYOld;
            if (0 != e.translationX || 0 != e.translationY) {
                transXOld = map.transX;
                transYOld = map.transY;
    ...
    bindElementEvents: function () {
        ...
        this.container.delegate("[class~='jvectormap-element']", "mouseover mouseout", function (e) {
            ...
            var type = -1 === baseVal.indexOf("jvectormap-region") ? "marker" : "region";
            var code = jvm.$(this).attr("region" == type ? "data-code" : "data-index");
            var element = "region" == type ? map.regions[code].element : map.markers[code].element;
            var tipText = "region" == type ? map.mapData.paths[code].name : map.markers[code].config.name || "";
        ...
        this.container.delegate("[class~='jvectormap-element']", "mouseup", function () {
            ...
            if (!mouseMoved) {
                ...
                if ("region" === type && map.params.regionsSelectable || "marker" === type && map.params.markersSelectable) {
    ...
    setScale: function (scale, anchorX, anchorY, isCentered, animate) {
        ...
        var that = this;
        var i = 0;
        var count = Math.abs(Math.round(60 * (scale - this.scale) / Math.max(scale, this.scale)));
        var deferred = new jvm.$.Deferred();
        ...
        if ("undefined" != typeof anchorX && "undefined" != typeof anchorY) {
    ...
    addMarker: function (key, marker, seriesData) {
        ...
        for (i = 0; i < seriesData.length; i++) {
            values = {};
            if ("undefined" != typeof seriesData[i]) {
    ...
    latLngToPoint: function (lat, lng) {
        ...
        if (-180 + centralMeridian > lng) {
            lng += 360;
        }
        point = jvm.Proj[proj.type](lat, lng, centralMeridian);
        inset = this.getInsetForPoint(point.x, point.y);
        return inset ? (bbox = inset.bbox, point.x = (point.x - bbox[0].x) / (bbox[1].x - bbox[0].x) * inset.width * this.scale, point.y = (point.y - bbox[0].y) / (bbox[1].y - bbox[0].y) * inset.height * this.scale, {
...
jvm.MultiMap.prototype = {
    addMap: function (name, config) {
        ...
        if (this.params.maxLevel > config.multiMapLevel) {
            this.maps[name].container.on("regionClick.jvectormap", {
                ...
                if (!(multimap.drillDownPromise && "pending" === multimap.drillDownPromise.state())) {
    ...
    drillDown: function (name, code) {
        ...
        var that = this;
        var focusPromise = currentMap.setFocus({
            region: code,
            animate: true
        });
        var downloadPromise = this.downloadMap(code);
        focusPromise.then(function () {
            if ("pending" === downloadPromise.state()) {
                that.spinner.show();
            }
        });
