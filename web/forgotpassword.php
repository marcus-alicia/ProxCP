    ...
    if ($validation->passed()) {
        ...
        if ($reset->exists()) {
            ...
            if ($current["mail_type"] == "sysmail") {
                $to = $reset->data()->email;
                $appname = parse_input($current["app_name"]);
                ...
                $from_email = parse_input($current["from_email"]);
                ...
                try {
                    $mail->isSMTP();
                    $mail->Host = parse_input($current["smtp_host"]);
                    $mail->SMTPAuth = true;
                    $mail->Username = parse_input($current["smtp_username"]);
                    $mail->Password = parse_input(_obfuscated_0D3C343005103213271D5C5B292F3D1D3D113836105B11_($current["smtp_password"]));
                    ...
                    $mail->Port = (int) parse_input($current["smtp_port"]);
                    $mail->setFrom(parse_input($current["from_email"]), parse_input($current["from_email_name"]));
                    $mail->addAddress($reset->data()->email);
                    $mail->addReplyTo(parse_input($current["from_email"]), parse_input($current["from_email_name"]));
                    $mail->isHTML(false);
                    $mail->Subject = parse_input($current["app_name"]) . " Password Reset Request";
                    $mail->Body = "Hello " . $reset->data()->username . ",\r\n\r\n    " . parse_input($current["app_name"]) . " has received a request to reset your password as it seems that you have forgotten it. Here is your new information:\r\n\r\n    Username: " . $reset->data()->username . "\r\n    Password: " . $string . "\r\n\r\n    Be sure you remember it!\r\n\r\n    Regards,\r\n    " . parse_input($current["app_name"]) . " Bot\r\n    URL: " . Config::get("instance/base");
                    $mail->send();
...
echo $twig->render("forgotpassword.tpl", ["appname" => $appname, "gethttps" => check_https(), "errors" => $errors, "formToken" => Token::generate(), "pagename" => "Forgot Password"]);
